...
import { getUserTranscationData } from '../../../store/userSlice';
...
function TranscationTable() {
  ...
  useEffect(() => {
    dispatch(getUserTranscationData(formVar.limit, formVar.offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const itemsLimitChange=(limit)=>{
    ...
    dispatch(getUserTranscationData(limit, formVar.offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const pageChange = (page) => {
    ...
    dispatch(getUserTranscationData(formVar.limit, offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  };
  ...
  const searchWithDelay = (e) => {
    ...
    const timer = setTimeout(() => {
      dispatch(getUserTranscationData(formVar.limit, formVar.offset, e.target.value, formVar.fromDate, formVar.toDate))
    }, typingDelay);
    setTypingTimer(timer);
  };
  const dateFilterChangeFronDate = (e) => {
    setFormVar((prevFormVar) => ({ ...prevFormVar, fromDate: e.target.value }))
    dispatch(getUserTranscationData(formVar.limit, formVar.offset, formVar.keyword, e.target.value, formVar.toDate))

  }
  const dateFilterChangeToDate = (e) => {
    setFormVar((prevFormVar) => ({ ...prevFormVar, toDate: e.target.value }))
    dispatch(getUserTranscationData(formVar.limit, formVar.offset, formVar.keyword, formVar.fromDate, e.target.value))
  }
      ...
      <Col sm='12'>
        <Card>
              ...
              <Col md="4" className='d-flex justify-content-end align-items-center gap-2'>
                <Col md="6">
                  <div className="text-end border-2 w-100">
                    <Btn attrBtn={{ color: 'info-gradien', className: 'w-100', size: 'sm', onClick: () => downloadExcel(storeVar.userTranscationData) }}>
                      Download as XLS
                    </Btn>
                  </div>
                </Col>
          ...
          <div className='table-responsive'>
            <Table hover={true} className='table-border-horizontal table-light'>
              ...
              <tbody>
                {storeVar.userTranscationData?.map((item, index) => (
                  <tr key={index}>
          ...
          {
            storeVar.userTranscationData?.length <= 0 && (
              <Empty />
            )
          }
        </Card>
        {
          ...
          <Pagination
            ...
            totalItem={storeVar.totalUserTranscation}
