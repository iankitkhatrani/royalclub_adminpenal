...
import { getRummyHistory } from '../../../store/rummySlice';

function RummyTable() {
  const storeVar = useSelector(state => state.rummy)
  ...
  useEffect(() => {
    dispatch(getRummyHistory(formVar.limit, formVar.offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const itemsLimitChange=(limit)=>{
    ...
    dispatch(getRummyHistory(limit, formVar.offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const pageChange = (page) => {
    ...
    dispatch(getRummyHistory(formVar.limit, offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const searchWithDelay = (e) => {
    ...
    const timer = setTimeout(() => {
      dispatch(getRummyHistory(formVar.limit, formVar.offset, e.target.value, formVar.fromDate, formVar.toDate))
    ...
    <Fragment>
      <Col sm='12'>
        <Card>
          ...
          {
            storeVar.rummyHistoryData?.length <= 0 && (
              <Empty />
            )
          }
        </Card>
        {
          storeVar.rummyHistoryData.length > 0 &&
          <Pagination currentPage={formVar.currentPage} totalItem={storeVar.totalRummyHistory} limitSelect={true}
