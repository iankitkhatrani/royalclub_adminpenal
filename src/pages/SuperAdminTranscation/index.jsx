...
import { getSuperAdminTranscationData } from '../../store/dashboardSlice';
...
function SuperAdminTranscation() {
  ...
  useEffect(() => {
    dispatch(getSuperAdminTranscationData(formVar.limit, formVar.offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const itemsLimitChange=(limit)=>{
    ...
    dispatch(getSuperAdminTranscationData(limit, formVar.offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const pageChange = (page) => {
    ...
    dispatch(getSuperAdminTranscationData(formVar.limit, offset, formVar.keyword, formVar.fromDate, formVar.toDate))
  ...
  const searchWithDelay = (e) => {
    ...
    const timer = setTimeout(() => {
      dispatch(getSuperAdminTranscationData(formVar.limit, formVar.offset, e.target.value, formVar.fromDate, formVar.toDate))
  ...
  const dateFilterChangeFronDate = (e) => {
    ...
    dispatch(getSuperAdminTranscationData(formVar.limit, formVar.offset, formVar.keyword, e.target.value, formVar.toDate))

  }
  const dateFilterChangeToDate = (e) => {
    setFormVar((prevFormVar) => ({ ...prevFormVar, toDate: e.target.value }))
    dispatch(getSuperAdminTranscationData(formVar.limit, formVar.offset, formVar.keyword, formVar.fromDate, e.target.value))
  }
  ...
  return (
    <Fragment>
      <Col sm='12'>
        <Card>
              ...
              <Col md="4" className='d-flex justify-content-end align-items-center gap-2'>
                <Col md="6">
                  <div className="text-end border-2 w-100">
                    <Btn attrBtn={{ color: 'info-gradien', className: 'w-100', size: 'sm', onClick: () => downloadExcel(storeVar.excelData) }}>
                      Download as XLS
                    </Btn>
                  </div>
                </Col>
          ...
          <div className='table-responsive'>
            <Table hover={true} className='table-border-horizontal table-light'>
              ...
              <tbody>
                {storeVar.transcationData?.map((item, index) => (
                  <tr key={index}>
          ...
          {
            storeVar.transcationData?.length <= 0 && (
              <Empty />
            )
          }
        </Card>
        {
          storeVar.transcationData.length > 0 &&
          <Pagination currentPage={formVar.currentPage} totalItem={storeVar.totatTranscation} limitSelect={true}
