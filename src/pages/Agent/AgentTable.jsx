...
// import { statusUpdateStaff } from '../../store/staffSlice';
// import CustomizerContext from '../../_helper/Customizer';
import Empty from '../../Components/Empty/Index';
import { getAgent, ModalToggle, isOpenModal, addAgent, updateAgent, deleteAgent } from '../../store/agentSlice';
...
const AgentTable = () => {
  ...
  useEffect(() => {
    dispatch(getAgent(formVar.limit, formVar.offset, formVar.status, formVar.keyword))
  }, []);
  const itemsLimitChange=(limit)=>{
    ...
    dispatch(getAgent(limit, formVar.offset, formVar.status, formVar.keyword))
  ...
  const pageChange = (page) => {
    ...
    dispatch(getAgent(formVar.limit, offset, formVar.status, formVar.keyword))
  };
  ...
  const searchWithDelay = (e) => {
    ...
    const timer = setTimeout(() => {
      dispatch(getAgent(formVar.limit, formVar.offset, formVar.status, e.target.value))
    }, typingDelay);
  ...
  const handleInputChange = (e) => {
    ...
    dispatch(getAgent(formVar.limit, formVar.offset, e.target.value, formVar.keyword))
  };
  ...
  // const submitStatus = () => {
  //   dispatch(statusUpdateStaff({ id: formVar.agentId, status: stateStatus==='ACTIVE'?true:false }))
  // }
  const nameValid = () => {
    if (!formVar.name) {
  ...
  const agentDelete = (data)=>{
    SweetAlert.fire({
      ...
      .then((result) => {
        if (result.value) {
          dispatch(deleteAgent({ id: data._id }))
        }
      });
  }
  // const emailValid = () => {
  ...
  return (
    <Fragment>
      <Col sm='12'>
        <Card>
          ...
          <div className='table-responsive'>
            <Table hover={true} className='table-border-horizontal table-light'>
              ...
              <tbody>
                {storeVar.agentData?.map((item, index) => (
                  <tr key={index}>
        ...
        {
          storeVar.agentData.length > 0 &&
          <Pagination currentPage={formVar.currentPage} totalItem={storeVar.totalAgent} limitSelect={true}
